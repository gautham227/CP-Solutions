...
int main() {
    ...
    for(int i=1;i<=t1;i++){
        ...
        cin>>s;
        cin>>t;
        int sca=0, scb=0, scc=0, tca=0, tcb=0, tcc=0;
        for(int i=0;i<n;i++){
            if(s[i]=='a')sca++;
            else if(s[i]=='b')scb++;
            else scc++;
        ...
        for(int i=0;i<sa.size();i++){
            v1.pb(make_pair(sa[i],ta[i]));
        }
        vector<pair<int,int>> v2;
        for(int i=0;i<sc.size();i++){
            v2.pb(make_pair(sc[i],tc[i]));
        }
        debug(v1)
        debug(v2)
        for(int i=0;i<v1.size();i++){
            auto it1=upper_bound(all(tc),v1[i].ff);
            auto it2=upper_bound(all(sc),v1[i].ff);
            // debug(*it1)
            // debug(*it2)
            if(v1[i].ff>v1[i].ss ||  (it2!=sc.end() && *it2<=v1[i].ss) || (it1!=tc.end() && *it1<=v1[i].ss)){
                flag=false;
                break;
            }
        }
        ...
        for(int i=0;i<v2.size();i++){
            auto it1=upper_bound(all(ta),v2[i].ss);
            auto it2=lower_bound(all(sa),v2[i].ss);
            // debug(*it1)
            // debug(*it2)
            if(v2[i].ff<v2[i].ss || (it2!=sa.end() && *it2<v2[i].ff) || (it1!=ta.end() && *it1<v2[i].ff)){
                flag=false;
                break;
            }
        }
