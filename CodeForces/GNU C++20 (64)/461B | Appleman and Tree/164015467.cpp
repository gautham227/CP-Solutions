...
void dfs(int ind, int par){
    ...
    if(colors[ind]==1){
        ...
        for(auto ch: adj[ind]){
            if(ch==par)continue;
            dp[ind][1]=mod_mul(dp[ind][1], dp[ch][0]+dp[ch][1], mod);
        }
    }
        ...
        for(auto ch: adj[ind]){
            if(ch==par)continue;
            dp[ind][0]=mod_mul(dp[ind][0], dp[ch][0]+dp[ch][1], mod);
        }
 
        for(auto ch: adj[ind]){
            if(ch==par)continue;
            dp[ind][1]=mod_add(dp[ind][1],mod_mul(dp[ch][1], mod_div(dp[ind][0],dp[ch][0]+dp[ch][1],mod),mod),mod);
        }
