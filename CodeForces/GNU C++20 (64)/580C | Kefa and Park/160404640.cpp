...
void dfs(int ind, vector<vector<int > >& adj, int catcnt, int mval, int par, vector<int>& cats){
    debug(ind)
    if(catcnt>mval)return;
    if(adj[ind].size()==1 && adj[ind][0]==par){
        if(mval>=catcnt){
            ans++;
        }
        return;
    ...
    for(auto x: adj[ind]){
        if(x==par)continue;
        if(cats[x]==1){
            dfs(x,adj,catcnt+1,mval,ind,cats);
        }
        else{
            dfs(x,adj,0,mval,ind,cats);
        }
    }
}
