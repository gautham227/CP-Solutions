...
ll recur(int ind, int k2used, vector<vector<ll> >&dp, vector<ll> &arr, int k){
    if(ind==arr.size()){
        return 0;
    }
    if(dp[ind][k2used]!=-1){
        return dp[ind][k2used];
    }
    ll ans=0;
    ll op1=(recur(ind+1,k2used,dp,arr,k))+((arr[ind]>>k2used)-k);
    ll op2=(recur(ind+1,min(30,k2used+1),dp,arr,k))+(arr[ind]>>(min(30,k2used+1)));
    ans=max(op1,op2);
    return dp[ind][k2used]=ans;
}
