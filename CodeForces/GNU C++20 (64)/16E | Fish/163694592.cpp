...
double recur(int val, vector<vector<double> >&mat, vector<double>& dp, int n, int ind){
    if(val==pow(2,n)-1)return 1;
    if(dp[val]!=-1)return dp[val];
    double curval=0;
    double prob1 = 1 / ((ind*(ind+1))/2.0) ;
    for(int i=0;i<n;i++){
        ...
        curval+=prob1*prob*recur((val|1<<i),mat,dp,n,ind+1);
    }
    return dp[val]=curval;
}
...
freopen("Error.txt", "w", stderr);
    ...
    for(int i=0;i<n;i++){
        cout<<fixed<<setprecision(10)<<recur(1<<i, mat,dp, n,1)<<" ";
    }
    cout<<endl;
