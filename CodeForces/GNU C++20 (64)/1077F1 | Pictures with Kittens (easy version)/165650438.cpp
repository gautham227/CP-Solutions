...
ll recur(ll ind, ll lind, ll rem, vector<ll>&a){
    if(rem==0){
        if(n-(lind)>=k)return -1e18;
        return 0;
    }
    if(ind-1==n-1){
        return a[ind-1];
    }
    if(dp[ind][lind][rem]!=-1)return dp[ind][lind][rem];
    ll val=0;
    if(ind-lind==k){
        val=a[ind-1]+recur(ind+1,ind,rem-1,a);
    }
    else{
        val=max(recur(ind+1,lind,rem,a), a[ind-1]+recur(ind+1,ind,rem-1,a));
    }
    return dp[ind][lind][rem]=val;
}
