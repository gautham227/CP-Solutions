...
ll base(ll ind, vector<ll>& a, vector<vector<ll> >&adj, ll d, ll par){
    // debug(ind)
    ll val=a[ind]*d;
    for(auto ch: adj[ind]){
        if(ch==par)continue;
        val+=base(ch,a,adj,d+1,ind);
    }
    return val;
}
 
void dfs1(ll vertex, ll parent, vector<vector<ll> >&adj, vector<ll> &subsum, vector<ll>& a){
    subsum[vertex]+=a[vertex];
    for(auto child: adj[vertex]){
        if(child==parent){
            continue;
        }
        dfs1(child, vertex,adj,subsum,a);
...
void dfs(ll ind, ll par, vector<vector<ll> >& adj, vector<ll> & a, vector<ll> & subsum, ll su, ll bas){
    ll curval=bas-(subsum[ind])+(su-subsum[ind]);
    // debug(curval)
    ans=max(ans,curval);
    // debug(adj[ind])
    for(auto ch: adj[ind]){
        ...
        if(ch==par)continue;
        dfs(ch,ind,adj,a,subsum,su, curval);
    }
}
...
int main() {
    ...
    dfs1(0,-1,adj,subsum,a);
    // debug(adj[0])
    for(auto x: adj[0]){
        dfs(x,0,adj,a,subsum,su,val);
    }
    // debug(subsum)
    cout<<ans<<endl;
