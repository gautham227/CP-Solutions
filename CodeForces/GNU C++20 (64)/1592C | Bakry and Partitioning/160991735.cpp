...
ll dfs(int node, vector<vector<ll> >&adj, vector<ll>&val, ll reqval, int par){
    ll vals=0;
    for(auto x: adj[node]){
        if(x!=par){
            vals^=dfs(x,adj,val,reqval,node);
        }
    }
    vals^=val[node];
    if(node!=1 && vals==reqval && flag==false){
        ele=node;
        paren=par;
        flag=true;
    }
...
int main() {
    ...
    while(t--)
    {
        ...
        adj[paren].erase(find(all(adj[paren]),ele));
        adj[ele].erase(find(all(adj[ele]),paren));
