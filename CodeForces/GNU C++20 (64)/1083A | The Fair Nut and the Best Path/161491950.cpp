...
ll dfs(ll ind, vector<vector<pll> >&adj, vector<ll>& cost, ll par){
    debug(ind)
    ll m1=0, m2=0;
    for(int i=0;i<adj[ind].size();i++){
        if(adj[ind][i].ff==par)continue;
        ll val=dfs(adj[ind][i].ff, adj, cost, ind)-adj[ind][i].ss;
        debug(val)
        if (val>m2)
        {
            m2=val;
