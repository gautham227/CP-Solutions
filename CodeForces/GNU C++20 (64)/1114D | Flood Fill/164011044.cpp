...
int recur(int l, int r, int col,vector<int>&arr, int n){
    if(r-l==n-1)return 0;
    if(dp[l][r][col]!=-1)return dp[l][r][col];
    int op1=INT_MAX;
    int op2=INT_MAX;
    if(l!=0){
        op1=recur(l-1,r,0,arr,n);
        if(col){
            op1+=(arr[l-1]!=arr[r]);
        }
        else{
