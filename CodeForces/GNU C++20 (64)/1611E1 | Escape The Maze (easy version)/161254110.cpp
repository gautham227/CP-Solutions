...
void dfs(int node, vector<vector<ll> > &adj, vector<ll> & time, int t, int par){
    if(time[node]<=t){
        ans++;
        return;
    }
    ...
    for(int i=0;i<adj[node].size();i++){
        ...
        dfs(adj[node][i],adj,time,t+1,node);
    }
}
...
freopen("Error.txt", "w", stderr);
    ...
    while(t--){
        ...
        vector<ll> cat; // index of all the ppl who are coming to catch him
        for(int i=0;i<k;i++){
            ll u;
            cin>>u;
            cat.pb(u);
