...
void pfind(int ind, int p, vector<int>& par, vector<vector<int> >& adj){
    par[ind]=p;
    for(auto x: adj[ind]){
        if(x==p)continue;
        pfind(x,ind,par,adj);
...
ll recur(int ind, int par, vector<vector<ll> >&dp, vector<vector<int> >&adj, int dist){
    if(dist==0)return 1;
    if(dist<0)return 0;
    if(dp[ind][dist]!=-1)return dp[ind][dist];
 
    int cur=0;
 
    for(auto ch: adj[ind]){
        if(ch==par)continue;
        cur+=recur(ch,ind,dp,adj,dist-1);
    }
 
    return dp[ind][dist]=cur;
...
freopen("Error.txt", "w", stderr);
    ...
    for(int i=0;i<n;i++){
        // subtree calculation
        ans+=recur(i,par[i],dp,adj,k);
        int p1=par[i];
        int d=k-1;
        int ch=i;
        // upwards
        while(p1!=-1 && d>-1){
            ans+=recur(p1,par[p1],dp,adj,d)-recur(ch,par[ch],dp,adj,d-1);
            ch=p1;
            p1=par[p1];
            d--;
        }
