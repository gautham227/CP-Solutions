...
int main() {
    ...
    for(int i=1;i<=n;i++){
        ll ele=v[i-1];
        for(int j=sieve[ele].size();j>=1;j--){
            
            if(sieve[ele][j-1]>n)continue;
            cur[sieve[ele][j-1]]=mod_add(cur[sieve[ele][j-1]],cur[sieve[ele][j-1]-1],mod);
        }
        // debug(cur)
    }
    ll ans=0;
    for(int i=1;i<=n;i++){
        ans=mod_add(ans,cur[i],mod);
    }
    cout<<ans<<endl;
    return 0;
}