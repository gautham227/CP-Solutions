...
ll recur(int val, int ind, vector<ll>& dp, int n, vector<vector<bool> >&mat){
    debug(ind)
    if(val==binexp(2,n,mod)-1){
        return 1;
    }
    if(dp[val]!=-1)return dp[val];
    ll curval=0;
    for(int i=0;i<n;i++){
        if(((val)&(1<<i))==0 && mat[ind][i]==1){
            curval=mod_add(recur(val|(1<<i),ind+1,dp,n,mat),curval, mod);
        }
    }
...
freopen("Error.txt", "w", stderr);
    ...
    int val=binexp(2,n,mod);

    vector<ll> dp(val,-1);
    ll ans=0;

    for(int i=0;i<n;i++){
        if(mat[0][i])ans=mod_add(recur(1<<i,1, dp, n, mat),ans,mod);
    }
