...
bool flag = false;
ll req;
string recur(int ind, int n, string& s,vector<vector<string> >&dp, vector<pii>& cards, int su){
    if(ind==n-1){
        if(su+cards[ind].ff==req){
            flag=true;
            s.pb('H');
            return dp[ind][su]=s;
        }
        else if(su+cards[ind].ss==req){
            flag=true;
            s.pb('T');
            return dp[ind][su]=s;
        }
    ...
    string s1=recur(ind+1,n,s,dp,cards,su+cards[ind].ff);
    if(flag){
        s.pb('H');
        return dp[ind][su]=s;
    }
    s1=recur(ind+1,n,s,dp,cards,su+cards[ind].ss);
    if(flag){
        s.pb('T');
        return dp[ind][su]=s;
    }
...
int main() {
    ...
    cin>>n>>s;
    req=s;
    vector<vector<string> >dp(101 , vector<string>(10001,"-1"));
    vector<pii> cards;
    for(int i=0;i<n;i++){
        int x,y;
        cin>>x>>y;
