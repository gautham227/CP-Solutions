...
bool dfs(ll u, vector<vector<ll> >&adj, vector<ll>&vis, ll res) {
    vis[u] = 1;
...
int main() {
    ...
    for(ll i=0;i<n;i++)
    {
        for(ll j=i+1;j<n;j++)
        {
            ll x1=cir[i][0],y1=cir[i][1],r1=cir[i][2];
            ll x2=cir[j][0],y2=cir[j][1],r2=cir[j][2];
    ...
    for(ll i=0;i<n;i++)
    {
        // debug(i)
        ll x1=cir[i][0],y1=cir[i][1],r1=cir[i][2];
        if((x1-sx)*(x1-sx)+(y1-sy)*(y1-sy)==1ll*r1*r1)
        {
            ind1=i;
        }
        if((x1-tx)*(x1-tx)+(y1-ty)*(y1-ty)==1ll*r1*r1)
        {
            ind2=i;
        }
    }
