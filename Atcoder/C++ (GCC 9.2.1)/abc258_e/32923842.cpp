...
int main() {
    ...
    while(1){
        ll scur=su-v[cur];
        if(x<=scur){
            int ind=lb(v,x+v[cur])-v.begin();
            if(s.find({cur+1,ind})==s.end()){
                s.insert({cur+1,ind});
                qv.pb({cur+1,ind});
                cnt.pb(ind-cur+n*tot);
            }
            else{
                st=cur+1;
                en=ind;
            ...
            scur=x-scur;
            int ind=lb(v,scur)-v.begin();
            if(s.find({cur+1,ind})==s.end()){
                s.insert({cur+1,ind});
                qv.pb({cur+1,ind});
                cnt.pb(ind+n-cur+n*tot);
            }
            else{
                st=cur+1;
                en=ind;
    ...
    while(q--){
        ...
        if(x<qv.size()){
            ...
            x-=qv.size();
            ll len=qv.size()-index;
            debug(len)
            x%=len;
            cout<<cnt[index+mod_sub(x,1,len)]<<endl;
        }
    }
