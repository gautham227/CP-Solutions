...
void recur(int ind, int tot, int req, vector<int>& v, vector<vector<int> >& v1){
    if(v.size()==req){
        v1.pb(v);
        return;
    }
...
freopen("Error.txt", "w", stderr);
    ...
    vector<int> v;
    recur(0,h1,h2,v,rows);
    v.clear();
    recur(0,w1,w2,v,columns);
    bool flag=false;
    for(int i=0;i<rows.size();i++){
        for(int j=0;j<columns.size();j++){
            bool f=true;
            for(int ind1=0;ind1<h2;ind1++){
                for(int ind2=0;ind2<w2;ind2++){
                    if(v2[ind1][ind2]!=v1[rows[i][ind1]][columns[j][ind2]]){
                        f=false;
                        break;
                    }
                }
