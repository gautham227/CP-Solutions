...
double n;
double recur(int a, int b, int c){
    if(a+b+c<0.00001)return 0;
    if(dp[a][b][c]!=0)return dp[a][b][c];
    double a1=(double)a, b1=(double)b, c1=(double)c;
    double val=0;
    val+=(n/(a1+b1+c1));
    if(a>0)val+=(a1/(a1+b1+c1))*(recur(a-1,b,c));
    if(b>0)val+=(b1/(a1+b1+c1))*(recur(a+1,b-1,c));
    if(c>0)val+=(c1/(a1+b1+c1))*(recur(a,b+1,c-1));
    return dp[a][b][c]=val;
}
...
freopen("Error.txt", "w", stderr);
    ...
    cout<<fixed<<setprecision(25)<<recur(c1,c2,c3)<<endl;
