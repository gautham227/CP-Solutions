...
int main() {
    ...
    vector<vector<ll>>d1,d2,d3;
    for(int i=1;i<=h[0];i++)
    {
        // vector<ll>v;
        for(int j=1;j<=h[0]-i-1;j++)
        {
    ...
    for(int i=0;i<d1.size();i++){
        for(int j=0;j<d2.size();j++){
            for(int k=0;k<d3.size();k++){
                if(d1[i][0]+d2[j][0]+d3[k][0]==w[0]&&d1[i][1]+d2[j][1]+d3[k][1]==w[1] && d1[i][2]+d2[j][2]+d3[k][2]==w[2]){
                    ans++;
                }
            }
        }
